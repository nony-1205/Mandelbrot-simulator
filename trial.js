...
// Creates an emulated double from a single precision float
vec2 ds_set(float a) {
    return vec2(a, 0.0);
}

// Emulated double-single addition (Two-Sum algorithm)
vec2 ds_add(vec2 dsa, vec2 dsb) {
    float s = dsa.x + dsb.x;
    float v = s - dsa.x;
    float e = (dsa.x - (s - v)) + (dsb.x - v) + dsa.y + dsb.y;
    float r_hi = s + e;
    float r_lo = e - (r_hi - s);
    return vec2(r_hi, r_lo);
}

// Emulated double-single multiplication (Two-Product algorithm variant)
vec2 ds_mul(vec2 dsa, vec2 dsb) {
    float c11 = dsa.x * dsb.x;
    float c21 = dsa.x * dsb.y + dsa.y * dsb.x;
    float t1 = c11 + c21;
    float t2 = c21 - (t1 - c11) + dsa.y * dsb.y;
    float r_hi = t1 + t2;
    float r_lo = t2 - (r_hi - t1);
    return vec2(r_hi, r_lo);
}

void main(){
    // Pixel delta calculated in single precision relative to view center
    vec2 pixel_delta = (gl_FragCoord.xy - u_res * 0.5) / (u_zoom * u_res.y * 0.5);

    // Initialize constant C with emulated double-single precision
    ...
    // Initialize Z coordinates as emulated doubles (0.0, 0.0)
    vec2 zx = vec2(0.0, 0.0);
    vec2 zy = vec2(0.0, 0.0);

    const int maxIter = 200;
    int escapeIter = maxIter;
    vec2 z_single = vec2(0.0, 0.0);

    for (int i = 0; i < maxIter; i++){
        // Compute high-precision squares: zx^2 and zy^2
        vec2 zx2 = ds_mul(zx, zx);
        vec2 zy2 = ds_mul(zy, zy);

        // Escape test checks the high-precision components
        if (zx2.x + zy2.x > 4.0) {
            escapeIter = i;
            z_single = vec2(zx.x, zy.x); // Save for smooth shading math
            break;
        }

        // zNew.y = 2.0 * zx * zy + cy
        vec2 zx_zy = ds_mul(zx, zy);
        vec2 zy_new = ds_add(ds_add(zx_zy, zx_zy), cy);

        // zNew.x = zx2 - zy2 + cx
        vec2 zx_new = ds_add(ds_add(zx2, vec2(-zy2.x, -zy2.y)), cx);

        zx = zx_new;
        zy = zy_new;
    }
        
    float dot_z = dot(z_single, z_single);
    float t = (escapeIter < maxIter)
        ? (float(escapeIter) - log2(log2(max(dot_z, 0.1))) + 4.0) / float(maxIter)
        : 0.0;
...
window.addEventListener('load',() =>{
    ...
    mandelbrot.canvas.addEventListener('wheel', (e)=>{
        ...
        const factor= delta > 0 ? 1/1.05 : 1.05 
